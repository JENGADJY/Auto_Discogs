...
def verif_data(Article, disc_csv):
    ...
    df = pd.read_csv(disc_csv , sep=",", on_bad_lines='warn')
    ...
    if df.empty == True:
        ...
        verif = []
        with open(disc_csv, newline='') as csvfile:
...
def verif_file(csv1,csv2):
    df1 = pd.read_csv(csv1 , sep=",", on_bad_lines='warn')
    df2 = pd.read_csv(csv2 , sep=",", on_bad_lines='warn')
    ...
    if rows_to_add_to_csv1:
        fieldnames = ['titre', 'artiste', 'formats', 'formats_discogs', 'year', 'labels', 'genres', 'styles']
        with open(csv1, 'a', newline='') as csvfile:  
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
